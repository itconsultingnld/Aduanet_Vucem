...
function excel_clientes($datos)
{
    ...
    $query = $CI->db->query('select * from clientes where Activo = 1 and empresa_id = '. $empresa_id);
    ...
    foreach ($query->result() as $row) {
        $array[0] = $row->empresaCliente;
        $array[1] = $row->nombreCliente . " " . $row->apePatCliente . " " . $row->apeMatCliente;
        $array[2] = $row->puestoCliente;
        $array[3] = $row->correoCliente;
        $array[4] = $row->numTelCliente;
